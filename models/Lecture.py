...
class Lecture(SQLModel, table=True):
    ...
    lecture_session: Annotated[int, Field(le=9, gt=0)]
    ...
    lecturer: Optional["Teacher"] = Relationship(
    ...
    classes: list["Class"] = Relationship(back_populates="lectures")
...
class LecturePost(SQLModel):
    model_config = model_config_dep
    name : Annotated[str, Field(title="Lecture name", description="Used for naming the Lecture")]
...
class LectureUpdate(SQLModel):
    model_config = model_config_dep
    name : Annotated[Optional[str], Field(title="Lecture name", description="Used for naming the Lecture")]
    lecture_dept : Annotated[Optional[int], Field(
        title="Lecture's origin dept.", 
        description="Used for declaring which department this class belongs to", 
        foreign_key="department.id")]
    lecture_code: Annotated[str | None, Field(max_length=20)]
    lecture_session: Annotated[int | None, Field(le=9, gt=0)]
    teacher_id: Annotated[int | None, Field(foreign_key="teacher.id", ondelete="CASCADE")]
    ects: int | None = Field(le=50, ge=0)