...
from sql import get_session, start_db
...
def login_for_access_token(
    # OAuth2PasswordRequestForm expects data formatted as 'application/x-www-form-urlencoded' 
    # (standard HTML forms) rather than a JSON body.
    form_data: Annotated[OAuth2PasswordRequestForm, Depends()],
    session: Annotated[Session, Depends(get_session)]
):
    # 1. Look up the user. (OAuth2 strictly requires the form field to be named 'username', 
    # but we can pass an email address into that field from the frontend).
    user = session.exec(select(User).where(User.email == form_data.username)).first()
    
    # 2. Verify identity. If the user doesn't exist OR the pwdlib verification fails, reject them.
    ...
    # 3. Create the token. "sub" (subject) is the standard JWT claim for the user's identifier.
    access_token = create_access_token(data={"sub": user.email})
    
    # 4. Return the token. The 'token_type': 'bearer' key is mandated by the OAuth2 specification.
    return {
